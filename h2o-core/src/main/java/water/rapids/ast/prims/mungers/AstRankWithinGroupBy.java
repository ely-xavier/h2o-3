...
public class AstRankWithinGroupBy extends AstPrimitive {
  ...
  public class RankGroups extends MRTask<RankGroups> {
    ...
    final IcedHashMap<GChunk, String>[] _chunkStasG;  // store all groupby class per chunk
    Frame _finalResult;

    private RankGroups(Frame inputFrame, int[] groupbycols, IcedHashMap<GChunk, String>[] chunkStas, int newRankCol) {
    ...
    public void map(Chunk[] chunks) {
      ...
      ArrayList<Integer> currentRank = new ArrayList<>(); // keep track of rank
      ...
      for (int rind=0; rind < chunks[0]._len; rind++) {
        if (!Double.isNaN(chunks[_newRankCol].atd(rind))) { // only rank when sorting columns contains no NAs
          ...
          if (keyIndex < 0) { // key not found before, add it here and set crank to 0
            ...
            currentRank.add(2); // +1 for next row
          } else {
    ...
    public void setStartRank(ArrayList<double[]> keys, ArrayList<Integer> vals, int cidx) {
      if ((cidx >0) && (_chunkStasG[cidx-1] != null)) {
        IcedHashMap<GChunk, String> temp = _chunkStasG[cidx-1];
        for (GChunk key: temp.keySet()) {
          keys.add(key._gs);
          vals.add(key._val+1);
        }
      }
  ...
  public class SortnGrouby extends MRTask<SortnGrouby> {
    ...
    IcedHashMap<GChunk, String>[] _chunkStasG;  // store all groupby class per chunk
    final int _groupbyLen;
    final int _newRankCol;
    ...
    public void map(Chunk[] chunks) {
      ...
      GChunk gWork = new GChunk(_groupbyLen, 0);
      GChunk gOld = null; // use for reference
      for (int rind = 0; rind < chunkLen; rind++) { // go through each row and groups them
        ...
        if (!foundNA) { // no NA in sort columns
          ...
          if (_chunkStasG[cidx].putIfAbsent(gWork, "") == null) { // Insert if not absent (note: no race, no need for atomic)
            gWork = new GChunk(_groupbyLen,1);   // need entirely new G
          } else {
      ...
      for (int ind = 0; ind < numChunks; ind++) {
        if (_chunkStasG[ind] == null) {
          if (git._chunkStasG[ind] != null && git._chunkStasG[ind].size() > 0) {
            ...
            for (GChunk key : git._chunkStasG[ind].keySet()) {
    ...
    public void postGlobal() {  // change counts per group per chunk to be cumulative
      ...
      ArrayList<Integer> cumCount = new ArrayList<>();

      for (int cInd = 0; cInd < _numChunks; cInd++) {
        IcedHashMap<GChunk, String> tempG = _chunkStasG[cInd];
        if (tempG != null) {
          for (GChunk key : tempG.keySet()) { // go through all keysets in current chunk
            int kindex = findKeyIndex(keys, key._gs);
            if (kindex >= 0) {
              int val = cumCount.get(kindex) + key._val;
              cumCount.set(kindex, val);
              GChunk gwork = tempG.getk(key);
              gwork._val = val;
        ...
        for (int ind=0; ind<currentArraySize;ind++) { // go through all keys in Arraylist so far
          ...
          if (tempG.putIfAbsent(gwork, "")==null) { // new element inserted
            gwork = new GChunk(_groupbyLen,0);
          }
        }
      }
    }
  ...
  public class GChunk extends Iced {
    ...
    public GChunk copyKey(double chks[], int val) {
    ...
    public boolean equals(Object o) { // count keys as equal if they have the same key values.
      return o instanceof GChunk && Arrays.equals(_gs, ((GChunk) o)._gs); // && _val==((GChunk) o)._val;
    }
