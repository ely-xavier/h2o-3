...
def test_rankWithGroupBy():
    ...
    rankedFrame = train.rankWithinGroupBy(groupCols, sortCols, sortDirs, newColName)
    rankedFrame.summary()
    finalFrame = rankedFrame.sort(finalSorts, finalDir)
    finalFrame.summary()
    pyunit_utils.compare_frames_local_onecolumn_NA(answerFrame[newColName], finalFrame[newColName],1, tol=1e-10)    # compare rank column

def generate_trainingFrame():
    train = pyunit_utils.random_dataset("regression", verbose=False, NTESTROWS=0)
    trainGroup = pyunit_utils.random_dataset_enums_only(train.nrows, randint(1,3), randint(2,10))
    trainEnums = pyunit_utils.random_dataset_numeric_only(train.nrows, randint(1,3), randint(20, 100))   # columns to sort
...
def generate_answerFrame(originalFrame, groupByCols, sortCols):
    ...
    sortFrames = answerFrame[sortCols[0]]
    for col in range(1,sortLen):
        sortFrames = sortFrames.cbind(answerFrame[sortCols[col]])
    tempS = sortFrames.as_data_frame(use_pandas=False)
    groupFrames = answerFrame[groupByCols[0]]
    for col in range(1, groupLen):
        groupFrames = groupFrames.cbind(answerFrame[groupByCols[col]])
